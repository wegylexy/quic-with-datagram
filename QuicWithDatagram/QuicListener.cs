...
public sealed partial class QuicListener : IAsyncDisposable
{
    ...
    private async void StartConnectionHandshake(QuicConnection connection, SslClientHelloInfo clientHello)
    {
        ...
        catch (OperationCanceledException oce) when (cancellationToken.IsCancellationRequested)
        {
            ...
            Exception ex = ExceptionDispatchInfo.SetCurrentStackTrace(new QuicException(QuicError.ConnectionTimeout, null, string.Format(SR.net_quic_handshake_timeout, QuicDefaults.HandshakeTimeout), oce));
            await connection.DisposeAsync().ConfigureAwait(false);
