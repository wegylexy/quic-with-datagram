...
public sealed class QuicListenerOptions
{
    ...
    internal void Validate(string argumentName)
    {
        ...
        {
            throw new ArgumentNullException(string.Format(SR.net_quic_not_null_listener, nameof(QuicListenerOptions.ListenEndPoint)), argumentName);
        }
        if (ApplicationProtocols is null || ApplicationProtocols.Count <= 0)
        {
            throw new ArgumentNullException(string.Format(SR.net_quic_not_null_not_empty_listener, nameof(QuicListenerOptions.ApplicationProtocols)), argumentName);
        }
        if (ListenBacklog == 0)
        {
        ...
        {
            throw new ArgumentNullException(string.Format(SR.net_quic_not_null_listener, nameof(QuicListenerOptions.ConnectionOptionsCallback)), argumentName);
        }
    }
}
