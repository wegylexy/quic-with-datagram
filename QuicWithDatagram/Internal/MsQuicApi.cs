    ...
    static MsQuicApi()
    {
        ...
        if (OperatingSystem.IsWindows())
        {
            if (IsWindowsVersionSupported())
            {
                ...
                if (!Tls13ServerMayBeDisabled && !Tls13ClientMayBeDisabled)
                {
                    // Windows ships msquic in the assembly directory.
                    UsesSChannelBackend = true;
                }
            }
            loaded = NativeLibrary.TryLoad(UsesSChannelBackend ? "msquic" : "msquic-openssl", typeof(MsQuicApi).Assembly, DllImportSearchPath.AssemblyDirectory, out msQuicHandle);
        }
        else
        {
            // Non-Windows relies on the package being installed on the system and may include the version in its name
            loaded = NativeLibrary.TryLoad("msquic", typeof(MsQuicApi).Assembly, null, out msQuicHandle);
        }
        ...
        try
        {
            ...
            MsQuicLibraryVersion = $"{"msquic"} {version} ({gitHash})";
            ...
            QUIC_TLS_PROVIDER provider = UsesSChannelBackend ? QUIC_TLS_PROVIDER.SCHANNEL : QUIC_TLS_PROVIDER.OPENSSL;
