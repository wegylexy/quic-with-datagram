...
public sealed partial class QuicConnection : IAsyncDisposable
{
    ...
    public static ValueTask<QuicConnection> ConnectAsync(QuicClientConnectionOptions options, CancellationToken cancellationToken = default)
    {
        if (!IsSupported)
        {
            throw new PlatformNotSupportedException(string.Format(SR.SystemNetQuic_PlatformNotSupported, MsQuicApi.NotSupportedReason));
        }
    ...
    private async ValueTask FinishConnectAsync(QuicClientConnectionOptions options, CancellationToken cancellationToken = default)
    {
        if (_connectedTcs.TryInitialize(out ValueTask valueTask, this, cancellationToken))
        {
            ...
            {
                throw new ArgumentException(string.Format(SR.net_quic_unsupported_endpoint_type, options.RemoteEndPoint.GetType()), nameof(options));
            }
    ...
    private int HandleEventDatagramStateChanged(ref DATAGRAM_STATE_CHANGED_DATA data)
    {
        DatagramMaxSendLength = data.MaxSendLength;
        return QUIC_STATUS_SUCCESS;
    }
    private unsafe int HandleEventDatagramReceived(ref DATAGRAM_RECEIVED_DATA data)
    {
        DatagramReceived?.Invoke(this, data.Buffer->Span);
        return QUIC_STATUS_SUCCESS;
    }
    private static unsafe int HandleEventDatagramSendStateChanged(ref DATAGRAM_SEND_STATE_CHANGED_DATA data)
        => GCHandle.FromIntPtr((IntPtr)data.ClientContext).Target is Func<QUIC_DATAGRAM_SEND_STATE, bool> callback && callback(data.State) ?
        QUIC_STATUS_SUCCESS :
...
#pragma warning restore CS3016
    ...
    {
        get => MsQuicHelpers.GetMsQuicParameter<byte>(_handle, QUIC_PARAM_CONN_DATAGRAM_RECEIVE_ENABLED) != 0;
        set => MsQuicHelpers.SetMsQuicParameter(_handle, QUIC_PARAM_CONN_DATAGRAM_RECEIVE_ENABLED, value ? (byte)1 : (byte)0);
    }
    ...
    {
        get => MsQuicHelpers.GetMsQuicParameter<byte>(_handle, QUIC_PARAM_CONN_DATAGRAM_SEND_ENABLED) != 0;
        set => MsQuicHelpers.SetMsQuicParameter(_handle, QUIC_PARAM_CONN_DATAGRAM_SEND_ENABLED, value ? (byte)1 : (byte)0);
    }
    ...
    public void SendDatagram(ReadOnlyMemory<byte> buffer, QuicDatagramSendOptions options = null)
        => SendDatagram(new ReadOnlySequence<byte>(buffer), options);
    ...
    /// <param name="options">Send options.</param>
    public unsafe void SendDatagram(ReadOnlySequence<byte> buffers, QuicDatagramSendOptions options = null)
    {
        int count = 0;
        ...
        MemoryHandle[] handles = new MemoryHandle[count];
        IntPtr quicBuffers = default;
        GCHandle callbackHandle = default;
        bool completed = false;
        Func<QUIC_DATAGRAM_SEND_STATE, bool> callback = state =>
        {
            bool TryComplete()
            {
                if (completed)
            ...
            var isFinal = (int)state >= (int)QUIC_DATAGRAM_SEND_STATE.LOST_DISCARDED;
            options?.StateChanged?.Invoke((QuicDatagramSendState)(int)state, isFinal);
            return state switch
            {
                QUIC_DATAGRAM_SEND_STATE.SENT or QUIC_DATAGRAM_SEND_STATE.LOST_SUSPECT => true,
                QUIC_DATAGRAM_SEND_STATE.LOST_DISCARDED or QUIC_DATAGRAM_SEND_STATE.ACKNOWLEDGED or QUIC_DATAGRAM_SEND_STATE.ACKNOWLEDGED_SPURIOUS or QUIC_DATAGRAM_SEND_STATE.CANCELED => TryComplete(),
                _ => isFinal && TryComplete() && false // unreachable
            };
        };
        callbackHandle = GCHandle.Alloc(callback);
        try
        {
            quicBuffers = Marshal.AllocHGlobal(sizeof(QUIC_BUFFER) * count);
            try
            {
                try
                {
                    QUIC_BUFFER* pointer = (QUIC_BUFFER*)quicBuffers;
                    count = 0;
                    foreach (ReadOnlyMemory<byte> buffer in buffers)
                    {
                        MemoryHandle handle = buffer.Pin();
                        QUIC_BUFFER* quicBuffer = &pointer[count];
                        quicBuffer->Length = (uint)buffer.Length;
                        quicBuffer->Buffer = (byte*)handle.Pointer;
                        handles[count] = handle;
                        ++count;
                    }
                    ThrowHelper.ThrowIfMsQuicError(MsQuicApi.Api.ApiTable->DatagramSend(
                        _handle.QuicHandle,
                        pointer,
                        (uint)count,
                        options.Priority ? QUIC_SEND_FLAGS.DGRAM_PRIORITY : QUIC_SEND_FLAGS.NONE,
                        (void*)(IntPtr)callbackHandle
                    ), "DatagramSend failed");
                }
                catch
                {
