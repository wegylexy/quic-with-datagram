        ...
        private static unsafe SafeMsQuicConfigurationHandle Create(QuicOptions options, QUIC_CREDENTIAL_FLAGS flags, X509Certificate? certificate, SslStreamCertificateContext? certificateContext, List<SslApplicationProtocol>? alpnProtocols)
        {
            ...
            var settings = new QuicSettings
            {
                IsSetFlags = QuicSettingsIsSetFlags.PeerBidiStreamCount |
                             ...
                             QuicSettingsIsSetFlags.DatagramReceiveEnabled,
                PeerBidiStreamCount = (ushort)options.MaxBidirectionalStreams,
                PeerUnidiStreamCount = (ushort)options.MaxUnidirectionalStreams
            };
            if (options.DatagramReceiveEnabled)
                settings.EnabledFlags |= QuicSettingsEnabledFlagsFlags.DatagramReceiveEnabled;
            ...
            if (options.MaxAckDelay != Timeout.InfiniteTimeSpan)
            {
                if (options.MaxAckDelay <= TimeSpan.Zero) throw new Exception("MaxAckDelay must not be negative.");
                uint ms = (uint)(options.MaxAckDelay.Ticks / TimeSpan.TicksPerMillisecond);

                settings.IsSetFlags |= QuicSettingsIsSetFlags.MaxAckDelayMs;
                settings.MaxAckDelayMs = ms;
            }
