...
namespace System.Net.Quic.Implementations.MsQuic
{
    internal sealed class MsQuicConnection : QuicConnectionProvider
    {
        ...
        internal sealed class State
        {
            ...
            public int DatagramMaxSendLength;
            ...
            public void ReceiveDatagram(ReadOnlySpan<byte> buffer) => DatagramReceived?.Invoke(Handle, buffer);
        }
        ...
        {
            state.ReceiveDatagram(connectionEvent.Data.DatagramReceived.Span);
            return MsQuicStatusCodes.Success;
        }
            ...
            try
            {
                switch (connectionEvent->Type)
                {
                    ...
                    case QUIC_CONNECTION_EVENT_TYPE.DATAGRAM_STATE_CHANGED:
                        return HandleEventDatagramStateChanged(state, ref *connectionEvent);
                    case QUIC_CONNECTION_EVENT_TYPE.DATAGRAM_RECEIVED:
                        return HandleEventDatagramReceived(state, ref *connectionEvent);
                    case QUIC_CONNECTION_EVENT_TYPE.DATAGRAM_SEND_STATE_CHANGED:
                        return HandleEventDatagramSendStateChanged(state, ref *connectionEvent);
                    default:
                        return MsQuicStatusCodes.Success;
                }
            }
        ...
        {
            get => MsQuicParameterHelpers.GetByteParam(MsQuicApi.Api, _state.Handle, QUIC_PARAM_LEVEL.CONNECTION, (uint)QUIC_PARAM_CONN.DATAGRAM_RECEIVE_ENABLED) != 0;
            set => MsQuicParameterHelpers.SetByteParam(MsQuicApi.Api, _state.Handle, QUIC_PARAM_LEVEL.CONNECTION, (uint)QUIC_PARAM_CONN.DATAGRAM_RECEIVE_ENABLED, (byte)(value ? 1 : 0));
        }

        internal override bool DatagramSendEnabled
        {
            get => MsQuicParameterHelpers.GetByteParam(MsQuicApi.Api, _state.Handle, QUIC_PARAM_LEVEL.CONNECTION, (uint)QUIC_PARAM_CONN.DATAGRAM_SEND_ENABLED) != 0;
            set => MsQuicParameterHelpers.SetByteParam(MsQuicApi.Api, _state.Handle, QUIC_PARAM_LEVEL.CONNECTION, (uint)QUIC_PARAM_CONN.DATAGRAM_SEND_ENABLED, (byte)(value ? 1 : 0));
        }

        internal override int DatagramMaxSendLength => _state.DatagramMaxSendLength;

        internal override event QuicDatagramReceivedEventHandler? DatagramReceived
        {
            add => _state.DatagramReceived += value;
